"""00_PLAN.md Phase 4 item 3's "scaling tests" (specs/0007-qp-allocation-stability/spec.md
AC-010): does compile+solve+verify complete, correctly, within a generous ceiling for a
moderate-scale QP request (hundreds of routes, one uniform desk-level allocation-stability penalty
applied to all of them). Not a tracked-baseline performance-regression suite, matching
`tests/benchmark/test_core_desk_scale.py`'s own precedent -- a much smaller scale than that LP
benchmark's 50,000 routes, since this test's purpose is confirming the QP path (quadratic
objective, PSD validation via the sparse ``eigsh`` code path, HiGHS Hessian translation and
scaling) stays fast and correct at a size well beyond a single hand-built fixture, not stress-
testing the solver's absolute scale ceiling.
...
@pytest.mark.slow
def test_qp_compile_and_solve_completes_within_time_ceiling(default_config) -> None:
    config = default_config.model_copy(
        update={"objective": ObjectiveConfig(allocation_stability_penalty=_PENALTY)}
    )
    ...
    assert elapsed < _WALL_CLOCK_CEILING_SECONDS, (
        f"QP scaling benchmark took {elapsed:.1f}s, exceeding the {_WALL_CLOCK_CEILING_SECONDS}s "
