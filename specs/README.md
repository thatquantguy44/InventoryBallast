...
| [0010-multi-period-settlement](0010-multi-period-settlement/) | Phase 5 item 2, deterministic form (`engine_spec/01_SPEC.md` §22.11): a projection of the already-solved period-0 book forward through known future trade events (recalls, returns, buys/sells/transfers already dated), producing per-period balances and discounted economics — not a re-optimized multi-period plan. Reuses `scenarios/apply.py`'s existing per-event-type mechanics unchanged. Explicitly defers §22.12's stochastic/scenario-tree extension and the full joint multi-period LP (a materially larger design, recorded as an alternative). | Draft — three open design questions need owner sign-off |
