...
| [0011-bloomberg-data-foundation](0011-bloomberg-data-foundation/) | Realism release R0 (`engine_spec/01_SPEC.md` §22.2-§22.4, T19-T21): point-in-time reference/calendar/corporate-action contracts, the Bloomberg adapter port boundary with a synthetic fixture-backed adapter (no real vendor SDK — entitlements unconfirmed), and opt-in pre-solve status/calendar validation. A hard prerequisite for R1 (T22-T24), not yet started. Targets `TRACEABILITY.md`'s `DAT-001`-`DAT-006` rows. | Proposed — pending owner sign-off on 3 open questions |
