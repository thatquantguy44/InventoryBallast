...
| LP-009 | MIP/QP/PWL/NLP features are explicit and capability-gated | 14, 16 | formulation/backend adapters | formulation mode | MIP portion (`all_or_none`, minimum-ticket, lot-size, cardinality; `formulation/mip.py`, `components/constraints/mip_rules.py`): `tests/golden/test_mip_business_rules.py`, `tests/unit/test_mip_compiler.py` (`specs/0006-mip-business-rules/`) | T15, T18 / R4-R7 / G4-G7 | `IMPLEMENTED` (MIP business rules only, per Section 14.1; QP/PWL/NLP -- Section 14.2-14.4 -- remain `SPECIFIED`, tagged T18 and later) |
