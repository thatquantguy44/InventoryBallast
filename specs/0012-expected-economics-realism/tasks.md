...
**Status note:** `spec.md` is **Proposed, not yet Approved** — four open questions need owner
sign-off (see `spec.md`'s Assumptions & Open Questions). **No task below has started.** Do not
begin T-001 until they are resolved and the spec's Status reads `Approved`, per the `0009`/`0010`/
`0011` precedent (each spent a sign-off round before any code was written).
...
| T-004 | Implement §22.9's fail-closed rule for hard entity-scoped limits (unresolved / low-confidence / conflicting), with a configurable `minimum_entity_confidence`; warn instead for non-hard limits. | REQ-004 | todo | Confirm Open Question 3 before writing this task's tests — it is the one place this spec deliberately departs from `0011`'s warnings-everywhere posture. |

### Slice B — expected economics (T23)

| ID | Task | Covers | Status | Notes |
| --- | --- | --- | --- | --- |
| T-005 | Add `domain/economics.py::ExpectedEconomics` with §22.5's coefficients plus the required `model_version`/`calibration_date`/`uncertainty` on every record. | REQ-005 | todo | Estimates are inputs; nothing in this repo produces them (NFR-003). |
| T-006 | Precompute `expected_active_fraction` (clamped to `[0, 1]`) and the additive expected costs in `formulation/context.py`; expose both on `BuildContext`. | REQ-006 | todo | Same slot and same discipline as `demand_caps`/`tier_caps`: computed once, before model construction. |
...
| T-008 | Add `ObjectiveConfig.economics_mode` (default `contractual`); wire `expected` mode through the compilers; fail closed when a route lacks an estimate in `expected` mode. | REQ-008, REQ-010, NFR-004 | todo | A config predating this field must remain valid and mean `contractual`. |
...
| T-013 | Update `specs/engine_spec/TRACEABILITY.md`: add `DAT-005` evidence and the §22.9 entity rows; **restore the `DAT-001`-`DAT-004`/`DAT-006` release tags `0011` incorrectly changed from `R3` to `R0`**, and add a one-line note naming which R-numbering the release column uses. | REQ-001 through REQ-015 | todo | See `spec.md`'s Open Question 4. This corrects a real error introduced by `0011`, so it lands whether or not the rest of this spec proceeds. |
| T-014 | Update `docs/handoff.md` and `specs/README.md`: record this spec as Implemented, state plainly that G3 remains un-granted and expected mode ships off, and restate what R1 still leaves open (PWL unwind cost, T25-T28). | REQ-001 through REQ-015 | todo | Last task. An earlier handoff/README edit already records this spec as *Proposed* (the `handoff-sync` gate requires it at spec-creation time) — this upgrades that entry. |
...
| AC-004 | `test_expected_economics_context.py::test_expected_active_fraction_formula_and_clamp` | todo |
| AC-005 | `tests/golden/test_expected_economics.py::test_expected_mode_shifts_allocation_to_higher_take_up` | todo |
| AC-006 | `test_expected_economics.py::test_contractual_mode_with_estimates_is_byte_identical` | todo |
| AC-007 | `test_expected_economics.py::test_expected_mode_fails_closed_on_missing_estimate` | todo |
| AC-008 | `test_expected_economics.py::test_attribution_reconciles_and_matches_hand_computed_expected_revenue` | todo |
...
- **T24's piecewise-linear liquidity/unwind cost** (§22.7's `PWL_i(v_i; knots)`) — proposed as its
  own spec (`spec.md` Open Question 2). Needs segment variables, a convexity argument (a convex
