...
- `specs/engine_spec/TRACEABILITY.md`'s `PLT-002` row and `docs/handoff.md`'s "Next priorities" section
...
| T-013 | Update `specs/engine_spec/TRACEABILITY.md`'s `PLT-002` row: add an evidence pointer to T-012's end-to-end test once it exists. Do **not** flip `PLT-002`'s status past `SPECIFIED` unilaterally — its `G2C` gate is a separate release approval this spec does not grant itself. | REQ-001 through REQ-013 | done | Confirmed `G2C` (`specs/engine_spec/ROADMAPS.md`) is a cross-cutting platform-ownership gate shared with `PLT-001`, `PLT-003`-`PLT-006` (all T17/T34-owned, none of which this repo builds) — `SPECIFIED` is correct to keep, not just cautious. |
